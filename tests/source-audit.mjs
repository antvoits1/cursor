...
check('Communications default width is 390px', has(app,'const DEFAULT_COMMS_WIDTH = 390;'));
check('Third panel is an iPhone frame', has(app,'IPhoneFrame') && has(css,'.iphone-bezel') && has(css,'.iphone-status-bar') && has(css,'.iphone-home-bar'));
check('Communications uses iOS bottom tabs', has(comm,'comm-tab-wrap') && has(css,'.comm-tabs button {') && has(css,'height: 49px;'));
...
check('Internal dialer code is absent', !/dialer|dtmf|keypad/i.test(app+nav+comm+css+store+leads+detail+messages+settings+notif+notifLib+format));
check('No call vendor SDK is bundled', !/voice-sdk|softphone|sip\.js|webrtc-adapter/i.test(JSON.stringify({ ...pkg.dependencies, ...pkg.devDependencies })));
check('Dialer and telephony modules are deleted from disk', absent('src/components/TopbarDialer.tsx') && absent('src/components/CallDock.tsx') && absent('src/lib/telephony.ts'));
