...
import { ArrowLeft, CheckCheck, Layers, Mail, MessageCircle, MessageSquare, Phone, PhoneIncoming, PhoneOutgoing, Reply, Search, Send, SquarePen, Users } from 'lucide-react';
...
function TabGlyph({ tab }: { tab: CommTab }) {
  if (tab === 'messages') return <MessageSquare size={20} strokeWidth={1.8}/>;
  if (tab === 'calls') return <Phone size={20} strokeWidth={1.8}/>;
  if (tab === 'contacts') return <Users size={20} strokeWidth={1.8}/>;
  if (tab === 'email') return <Mail size={20} strokeWidth={1.8}/>;
  return <Layers size={20} strokeWidth={1.8}/>;
}
    ...
    <div className={`comm-panel${inDrillDown ? ' drill' : ''}`}>
      <header className={`comm-head${inDrillDown ? ' compact' : ' large'}`}>
        <div className="comm-head-side">
          ...
          {inDrillDown && activeTab === 'calls' && <button type="button" className="comm-contact-back" onClick={() => setOpenCall(null)}><ArrowLeft size={16}/> Calls</button>}
          ...
          {inDrillDown && activeTab === 'email' && <button type="button" className="comm-contact-back" onClick={() => { setOpenEmailKey(null); setEmailLeadId(null); setEmailSubject(''); setEmailBody(''); }}><ArrowLeft size={16}/> Email</button>}
        </div>
      ...
      <div className="comm-scroll">
        {activeTab === 'all' && (
          <div className="comm-list">
            <div className="comm-list-label">Latest communications</div>
            {!allItems.length && <div className="comm-empty">No communications on file.</div>}
            {allItems.map(item => (
              <button type="button" key={item.key} className="comm-row" onClick={() => openAllItem(item)}>
                <span className={`comm-type-icon ${item.type}`}><TypeIcon type={item.type}/></span>
                <span className="comm-row-copy">
                  <span className="comm-row-top"><strong>{item.title}</strong><time>{item.when}</time></span>
                  <span className="comm-row-sub">{item.lead.company}</span>
                  <span className="comm-row-detail">{item.detail}</span>
                </span>
              </button>
            ))}
          </div>
        ...
        {activeTab === 'messages' && !currentMessageLead && (
          <div className="comm-list">
            <div className="comm-list-label">Messages</div>
        ...
        {activeTab === 'calls' && !openCall && (
          <div className="comm-list">
            <div className="comm-list-label">Recent calls</div>
      ...
      {activeTab === 'messages' && currentMessageLead && (
        <div className="comm-composer">
          ...
          <textarea ref={composerRef} value={messageText} onChange={e => setMessageText(e.target.value)} onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); sendMessage(); } }} placeholder={messageChannel === 'wa' ? 'WhatsApp message' : 'Message'} rows={1}/>
          <button type="button" onClick={sendMessage} disabled={!messageText.trim() || !currentMobile} title="Send" className="comm-send"><Send size={15}/></button>
        </div>
      )}
      {activeTab === 'email' && (openEmailKey || emailLeadId) && (
        <div className="comm-email-compose">
          <div className="comm-email-address-row"><span>To</span><strong>{activeEmailLead?.emails?.[0]?.n || 'No email'}</strong></div>
          ...
          <textarea ref={replyRef} value={emailBody} onChange={e => setEmailBody(e.target.value)} placeholder={activeEmail ? 'Reply…' : 'Write email…'}/>
          <div className="comm-email-actions">
            <span className="comm-email-from">sales@forgecrm.com</span>
            <button type="button" onClick={sendEmail} disabled={!emailBody.trim() || !activeEmailLead?.emails?.[0]?.n} className="comm-send"><Send size={14}/></button>
          </div>
        </div>
      )}
      ...
      {!inDrillDown && (
        <div className="comm-tab-wrap">
          <nav className="comm-tabs">
            {(['all','messages','calls','contacts','email'] as CommTab[]).map(tab => (
